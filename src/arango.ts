...
import { ArangoSmoothieError } from './exceptions/errors';
import { parseError } from './utils/parse-errors';
import { generateUUID } from './utils/generate-uuid';
import * as connection from "arangojs/connection";
import { ModelTypes } from "./model/model.types";

export interface ConnectOptions extends connection.Connection {
}

interface ArangoSmoothieConfig {
...
export type ArangoSmoothieEvents = 'IndexOnline';

/**
 * Store default connection.
 */
export let __arangoSmoothie: ArangoSmoothie;
export let __arangoSmoothieInstances: ArangoSmoothie[] = [];

export class ArangoSmoothie {
  ...
  // private events: Record<ArangoSmoothieEvents, NodeCallback<ArangoSmoothie>[]> = {
  //   IndexOnline: [],
  // };
  ...
  /**
   * Register ArangoSmoothie events
   ...
   */
  on(event: ArangoSmoothieEvents, fn: NodeCallback<ArangoSmoothie>) {
    switch (event) {
      case 'IndexOnline':
  ...
  constructor(config: ArangoSmoothieConfig = {}) {
    ...
    if (!__arangoSmoothie) {
      ...
      __arangoSmoothie = this;
    }
    __arangoSmoothieInstances.push(this);
  }
   ...
   * @example
   * ```javascript
   *  import { connect } from "arangoSmoothie";
   *  const connection = connect("couchbase://localhost/travel-sample@admin:password");
   * ```
   */
  connect = async (connectOptions: ConnectOptions | string): Promise<ArangoSmoothie> => {
  ...
  model<T = any, R = T>(
    ...
    if (this.models[name]) {
      throw new ArangoSmoothieError(`A model with name '${name}' has already been registered.`);
    }
    ...
    const ModelFactory = createModel<T, R>({ name, schemaDraft: schema, options: modelOptions, arangoSmoothie: this });
  ...
  async close(): Promise<void> {
    if (this.cluster) {
      ...
      __arangoSmoothieInstances = __arangoSmoothieInstances.filter((instance) => instance.id !== this.id);
      if (__arangoSmoothie?.id === this.id) {
        (__arangoSmoothie as any) = undefined;
      }
    }
  }
   ...
   *
   * ArangoSmoothie provides a powerful [Query Builder](docs/basic/query-builder) system to create valid N1QL queries using method chaining.
   ...
   *
   * Notice: It's not required to execute the `start` method in order for ArangoSmoothie work.
   */
...
export const getDefaultInstance = () => __arangoSmoothie;
export const getArangoSmoothieInstances = () => __arangoSmoothieInstances;
export const connect = async (connectOptions: ConnectOptions | string) => {
  if (!__arangoSmoothie) {
    new ArangoSmoothie();
  }
  __arangoSmoothie && (await __arangoSmoothie.connect(connectOptions));
};
export const close = async (): Promise<void> => {
  if (__arangoSmoothie) {
    await __arangoSmoothie.close();
  }
};
export const start = () => __arangoSmoothie?.start();
export const getModel = (name: string) => __arangoSmoothie?.getModel(name);
export const getCollection = (collectionName = DEFAULT_COLLECTION, scopeName = DEFAULT_SCOPE) =>
  __arangoSmoothie?.getCollection(collectionName, scopeName);
export const model = <T = any, R = T>(
  name: string,
  schema: Schema | Record<string, unknown>,
  options?: ModelOptions,
): ModelTypes<T, R> => __arangoSmoothie?.model<T, R>(name, schema, options);

// export const searchQuery = (
//   indexName: string,
//   query: SearchQuery,
//   options?: SearchQueryOptions,
// ): StreamableRowPromise<SearchResult, SearchRow, SearchMetaData> => __arangoSmoothie?.searchQuery(indexName, query, options);
